...
// BmwX5Seeder seeds specifications for BMW X5
type BmwX5Seeder struct {
	BaseSeeder
}

// NewBmwX5Seeder creates a new BMW X5 seeder
func NewBmwX5Seeder() *BmwX5Seeder {
	return &BmwX5Seeder{
		BaseSeeder: BaseSeeder{name: "BMW X5 Specifications"},
	}
}

func (bx5s *BmwX5Seeder) getBanglaTranslations() map[string]string {
...
// Seed implements the Seeder interface for BMW X5
func (bx5s *BmwX5Seeder) Seed(db *gorm.DB) error {
	productSlug := "bmw-x5"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := bx5s.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ BMW X5 specifications seeded successfully")
	return nil
}
