package seeders

import (
	"kossti/internal/infrastructure/database/models"
	"log"

	"gorm.io/gorm"
)

// MicrosoftSurfaceLaptopStudio2Seeder seeds specifications for Microsoft Surface Laptop Studio 2
type MicrosoftSurfaceLaptopStudio2Seeder struct {
	BaseSeeder
}

// NewMicrosoftSurfaceLaptopStudio2Seeder creates a new Microsoft Surface Laptop Studio 2 seeder
func NewMicrosoftSurfaceLaptopStudio2Seeder() *MicrosoftSurfaceLaptopStudio2Seeder {
	return &MicrosoftSurfaceLaptopStudio2Seeder{
		BaseSeeder: BaseSeeder{name: "Microsoft Surface Laptop Studio 2 Specifications"},
	}
}

func (msss *MicrosoftSurfaceLaptopStudio2Seeder) getBanglaTranslations() map[string]string {
	return map[string]string{
		"Microsoft":                     "মাইক্রোসফ্ট",
		"Surface Laptop Studio 2":       "সারফেস ল্যাপটপ স্টুডিও ২",
		"Windows 11 Home":               "উইন্ডোজ ১১ হোম",
		"Intel":                         "ইন্টেল",
		"Core i7-13700H":                "কোর আই৭-১৩৭০০এইচ",
		"13th Gen":                      "১৩তম প্রজন্ম",
		"Intel HM770":                   "ইন্টেল এইচএম৭৭০",
		"NVIDIA GeForce RTX 4050":       "এনভিডিয়া গেফোর্স আরটিএক্স ৪০৫০",
		"2400 x 1600 pixels":            "২৪০০ x ১৬০০ পিক্সেল",
		"14.4 inches":                   "১৪.৪ ইঞ্চি",
		"58 Wh":                         "৫৮ ডব্লিউএইচ",
		"Aluminum":                      "অ্যালুমিনিয়াম",
		"Platinum":                      "প্লাটিনাম",
		"1.91 kg":                       "১.৯১ কেজি",
		"2024":                          "২০২৪",
		"1 Year International Warranty": "১ বছর আন্তর্জাতিক ওয়ারেন্টি",
		"1 TB":                          "১ টেরাবাইট",
		"Bluetooth 5.3":                 "ব্লুটুথ ৫.৩",
		"32 GB":                         "৩২ জিবি",
		"IPS":                           "আইপিএস",
		"120 Hz":                        "১২০ হার্জ",
		"Yes":                           "হ্যাঁ",
		"3.5mm Combo Jack":              "৩.৫মিমি কম্বো জ্যাক",
		"Wi-Fi 6E (802.11ax)":           "ওয়াই-ফাই ৬ই (৮০২.১১এক্স)",
		"USB 3.2 Gen 2":                 "ইউএসবি ৩.২ জেন ২",
		"Dedicated":                     "ডেডিকেটেড",
		"1 TB SSD":                      "১ টেরাবাইট এসএসডি",
		"Intel Core i7":                 "ইন্টেল কোর আই৭",
		"Dual Fan":                      "ডুয়াল ফ্যান",
		"120":                           "১২০",
		"No":                            "না",
		"1 Year":                        "১ বছর",
		"2-in-1 Laptop":                 "২-ইন-১ ল্যাপটপ",
		"Lithium-ion":                   "লিথিয়াম-আয়ন",
		"102W Surface Connect":          "১০২ওয়াট সারফেস কানেক্ট",
		"18 hours":                      "১৮ ঘন্টা",
		"IPS, 400 nits":                 "আইপিএস, ৪০০ নিটস",
		"2.4 GHz base / 5.0 GHz boost":  "২.৪ গিগাহার্টজ বেস / ৫.০ গিগাহার্টজ বুস্ট",
		"24 MB cache":                   "২৪ এমবি ক্যাশ",
		"1080p FHD":                     "১০৮০পি এফএইচডি",
		"Quad Speakers":                 "কোয়াড স্পিকার",
		"323 x 228 x 22.5 mm":           "৩২৩ x ২২৮ x ২২.৫ মিমি",
		"TPM 2.0":                       "টিপিএম ২.০",
		"14":                            "১৪",
		"20":                            "২০",
		"5200 MHz":                      "৫২০০ মেগাহার্টজ",
		"NVMe PCIe Gen4":                "এনভিএমই পিসিআই জেন৪",
		"6 GB GDDR6":                    "৬ জিবি জিডিডিআর৬",
		"English":                       "ইংরেজি",
		"2x USB-C 3.2 Gen 2, 1x Surface Connect, 1x SD Card": "২x ইউএসবি-সি ৩.২ জেন ২, ১x সারফেস কানেক্ট, ১x এসডি কার্ড",
	}
}

// Seed implements the Seeder interface for Microsoft Surface Laptop Studio 2
func (msss *MicrosoftSurfaceLaptopStudio2Seeder) Seed(db *gorm.DB) error {
	productSlug := "microsoft-surface-laptop-studio-2"
	var prod models.ProductModel
	if err := db.Where("slug = ?", productSlug).First(&prod).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Printf("⚠️  Product not found: %s", productSlug)
			return nil
		}
		return err
	}
	productID := prod.ID

	// Mapping of specification keys to their IDs in the database
	existingKeyMapping := map[string]uint{
		"Brand":                   310,
		"Model Name":              316,
		"Operating System":        49,
		"Processor Brand":         204,
		"Processor Model":         206,
		"Processor Generation":    205,
		"Chipset":                 19,
		"Graphics Card":           200,
		"Screen Resolution":       207,
		"Display Size":            683,
		"Battery Capacity":        11,
		"Build Material":          14,
		"Color":                   318,
		"Product Weight":          697,
		"Release Year":            317,
		"Warranty":                323,
		"Storage Capacity":        71,
		"Bluetooth Version":       13,
		"RAM":                     60,
		"Weight":                  80,
		"Display Type":            27,
		"Refresh Rate":            61,
		"Screen Size":             66,
		"Backlit Keyboard":        199,
		"Audio Jack":              682,
		"Ram":                     684,
		"Wifi Support":            687,
		"Usb Type":                688,
		"Battery":                 689,
		"Gpu Type":                691,
		"Storage":                 693,
		"Cpu Type":                696,
		"Cooling Technology":      698,
		"Frequency (Hz)":          704,
		"App Control":             705,
		"Warranty Period":         706,
		"Laptop Type":             24,
		"Battery Type":            12,
		"Charging Speed":          18,
		"Standby Time":            70,
		"Wireless Charging":       82,
		"Resolution":              62,
		"Display Characteristics": 26,
		"Processor Speed":         57,
		"Clock Feature":           20,
		"3.5mm Audio Jack":        2,
		"Camera Features":         16,
		"Audio Quality":           9,
		"Sensors":                 67,
		"Dimensions":              25,
		"Body Type":               89,
		"Cooling System":          95,
		"Available Colors":        10,
		"Special Features":        69,
		"Processor Cores":         786,
		"Processor Threads":       787,
		"RAM Speed":               788,
		"RAM Slots":               789,
		"RAM Expandable":          790,
		"Storage Interface":       791,
		"Storage Expandable":      792,
		"Graphics VRAM":           793,
		"Display Touch Support":   794,
		"Ethernet":                795,
		"Thunderbolt Version":     796,
		"SD Card Reader":          797,
		"Keyboard Language":       798,
		"Build Standard":          799,
		"Touchscreen":             129,
		"HDMI Ports":              167,
		"USB Ports":               173,
	}

	specs := map[string]string{
		"Brand":                   "Microsoft",
		"Model Name":              "Surface Laptop Studio 2",
		"Operating System":        "Windows 11 Home",
		"Processor Brand":         "Intel",
		"Processor Model":         "Core i7-13700H",
		"Processor Generation":    "13th Gen",
		"Chipset":                 "Intel HM770",
		"Graphics Card":           "NVIDIA GeForce RTX 4050",
		"Screen Resolution":       "2400 x 1600 pixels",
		"Display Size":            "14.4 inches",
		"Battery Capacity":        "58 Wh",
		"Build Material":          "Aluminum",
		"Color":                   "Platinum",
		"Product Weight":          "1.91 kg",
		"Release Year":            "2024",
		"Warranty":                "1 Year International Warranty",
		"Storage Capacity":        "1 TB",
		"Bluetooth Version":       "Bluetooth 5.3",
		"RAM":                     "32 GB",
		"Weight":                  "1.91 kg",
		"Display Type":            "IPS",
		"Refresh Rate":            "120 Hz",
		"Screen Size":             "14.4 inches",
		"Backlit Keyboard":        "Yes",
		"Audio Jack":              "3.5mm Combo Jack",
		"Ram":                     "32 GB",
		"Wifi Support":            "Wi-Fi 6E (802.11ax)",
		"Usb Type":                "USB 3.2 Gen 2",
		"Battery":                 "58 Wh",
		"Gpu Type":                "Dedicated",
		"Storage":                 "1 TB SSD",
		"Cpu Type":                "Intel Core i7",
		"Cooling Technology":      "Dual Fan",
		"Frequency (Hz)":          "120",
		"App Control":             "No",
		"Warranty Period":         "1 Year",
		"Laptop Type":             "2-in-1 Laptop",
		"Battery Type":            "Lithium-ion",
		"Charging Speed":          "102W Surface Connect",
		"Standby Time":            "18 hours",
		"Wireless Charging":       "No",
		"Resolution":              "2400 x 1600 pixels",
		"Display Characteristics": "IPS, 400 nits",
		"Processor Speed":         "2.4 GHz base / 5.0 GHz boost",
		"Clock Feature":           "24 MB cache",
		"3.5mm Audio Jack":        "Yes",
		"Camera Features":         "1080p FHD",
		"Audio Quality":           "Quad Speakers",
		"Sensors":                 "No",
		"Dimensions":              "323 x 228 x 22.5 mm",
		"Body Type":               "Aluminum",
		"Cooling System":          "Dual Fan",
		"Available Colors":        "Platinum",
		"Special Features":        "TPM 2.0",
		"Processor Cores":         "14",
		"Processor Threads":       "20",
		"RAM Speed":               "5200 MHz",
		"RAM Slots":               "No",
		"RAM Expandable":          "No",
		"Storage Interface":       "NVMe PCIe Gen4",
		"Storage Expandable":      "No",
		"Graphics VRAM":           "6 GB GDDR6",
		"Display Touch Support":   "Yes",
		"Ethernet":                "No",
		"Thunderbolt Version":     "4",
		"SD Card Reader":          "Yes",
		"Keyboard Language":       "English",
		"Build Standard":          "Standard",
		"Touchscreen":             "Yes",
		"HDMI Ports":              "No",
		"USB Ports":               "2x USB-C 3.2 Gen 2, 1x Surface Connect, 1x SD Card",
	}

	// Create specifications
	for key, value := range specs {
		keyID, exists := existingKeyMapping[key]
		if !exists {
			log.Printf("⚠️  Specification key not found: %s", key)
			continue
		}

		spec := models.SpecificationModel{
			ProductID:          productID,
			SpecificationKeyID: keyID,
			Value:              value,
		}

		// Check if specification already exists
		var existingSpec models.SpecificationModel
		if err := db.Where("product_id = ? AND specification_key_id = ?", productID, keyID).First(&existingSpec).Error; err != nil {
			if err == gorm.ErrRecordNotFound {
				// Create new specification
				if err := db.Create(&spec).Error; err != nil {
					log.Printf("❌ Failed to create specification for key %s: %v", key, err)
					continue
				}
			} else {
				log.Printf("❌ Error checking existing specification for key %s: %v", key, err)
				continue
			}
		} else {
			log.Printf("ℹ️  Specification already exists for key %s, skipping", key)
			continue
		}

		// Create translation if Bangla translation exists
		if banglaValue, exists := msss.getBanglaTranslations()[value]; exists {
			translation := models.SpecificationTranslationModel{
				SpecificationID: spec.ID,
				Locale:          "bn",
				Value:           banglaValue,
			}

			if err := db.Create(&translation).Error; err != nil {
				log.Printf("❌ Failed to create translation for key %s: %v", key, err)
			}
		}
	}

	log.Printf("✅ Microsoft Surface Laptop Studio 2 specifications seeded successfully")
	return nil
}
