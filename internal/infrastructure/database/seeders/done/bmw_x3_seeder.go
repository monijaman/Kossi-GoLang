...
// BmwX3Seeder seeds specifications for BMW X3
type BmwX3Seeder struct {
	BaseSeeder
}

// NewBmwX3Seeder creates a new BMW X3 seeder
func NewBmwX3Seeder() *BmwX3Seeder {
	return &BmwX3Seeder{
		BaseSeeder: BaseSeeder{name: "BMW X3 Specifications"},
	}
}

func (bx3s *BmwX3Seeder) getBanglaTranslations() map[string]string {
...
// Seed implements the Seeder interface for BMW X3
func (bx3s *BmwX3Seeder) Seed(db *gorm.DB) error {
	productSlug := "bmw-x3"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := bx3s.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ BMW X3 specifications seeded successfully")
	return nil
}
