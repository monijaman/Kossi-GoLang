...
// GMCTerrainSeeder seeds specifications for GMC Terrain
type GMCTerrainSeeder struct {
	BaseSeeder
}

// NewGMCTerrainSeeder creates a new GMC Terrain seeder
func NewGMCTerrainSeeder() *GMCTerrainSeeder {
	return &GMCTerrainSeeder{
		BaseSeeder: BaseSeeder{name: "GMC Terrain Specifications"},
	}
}

func (gts *GMCTerrainSeeder) getBanglaTranslations() map[string]string {
...
// Seed implements the Seeder interface for GMC Terrain
func (gts *GMCTerrainSeeder) Seed(db *gorm.DB) error {
	productSlug := "gmc-terrain"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := gts.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ GMC Terrain specifications seeded successfully")
	return nil
}
