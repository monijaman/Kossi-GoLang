...
// GMCYukonSeeder seeds specifications for GMC Yukon
type GMCYukonSeeder struct {
	BaseSeeder
}

// NewGMCYukonSeeder creates a new GMC Yukon seeder
func NewGMCYukonSeeder() *GMCYukonSeeder {
	return &GMCYukonSeeder{
		BaseSeeder: BaseSeeder{name: "GMC Yukon Specifications"},
	}
}

func (gys *GMCYukonSeeder) getBanglaTranslations() map[string]string {
...
// Seed implements the Seeder interface for GMC Yukon
func (gys *GMCYukonSeeder) Seed(db *gorm.DB) error {
	productSlug := "gmc-yukon"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := gys.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ GMC Yukon specifications seeded successfully")
	return nil
}
