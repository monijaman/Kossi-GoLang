...
// NissanXTtrailSeeder seeds specifications for Nissan X-Trail
type NissanXTtrailSeeder struct {
	BaseSeeder
}

// NewNissanXTtrailSeeder creates a new Nissan X-Trail seeder
func NewNissanXTtrailSeeder() *NissanXTtrailSeeder {
	return &NissanXTtrailSeeder{
		BaseSeeder: BaseSeeder{name: "Nissan X-Trail Specifications"},
	}
}

func (nxts *NissanXTtrailSeeder) getBanglaTranslations() map[string]string {
	return map[string]string{
		"X-Trail 2.0":             "এক্স-ট্রেইল ২.০",
		"4th Generation (T33)":    "৪র্থ প্রজন্ম (টি৩৩)",
...
// Seed implements the Seeder interface for Nissan X-Trail
func (nxts *NissanXTtrailSeeder) Seed(db *gorm.DB) error {
	productSlug := "nissan-x-trail"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := nxts.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ Nissan X-Trail specifications seeded successfully")
	return nil
}
