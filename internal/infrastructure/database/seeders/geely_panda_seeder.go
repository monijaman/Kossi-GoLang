...
// GeelyPandaSeeder seeds specifications for Geely Panda
type GeelyPandaSeeder struct {
	BaseSeeder
}

// NewGeelyPandaSeeder creates a new Geely Panda seeder
func NewGeelyPandaSeeder() *GeelyPandaSeeder {
	return &GeelyPandaSeeder{
		BaseSeeder: BaseSeeder{name: "Geely Panda Specifications"},
	}
}

func (gps *GeelyPandaSeeder) getBanglaTranslations() map[string]string {
...
// Seed implements the Seeder interface for Geely Panda
func (gps *GeelyPandaSeeder) Seed(db *gorm.DB) error {
	productSlug := "geely-panda"
	...
	for key, value := range specs {
		...
		if banglaValue, exists := gps.getBanglaTranslations()[value]; exists {
	...
	log.Printf("✅ Geely Panda specifications seeded successfully")
	return nil
}
