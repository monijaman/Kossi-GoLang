...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: go run enhance_batch_seeders.go <seeder_file.go>")
		fmt.Println("Example: go run enhance_batch_seeders.go internal/infrastructure/database/seeders/lifan_kpv_150_batch28_seeder.go")
		os.Exit(1)
	}
	...
	originalLength := len(strings.Split(fileContent, "\n"))

	// Enhance content
	enhanced := enhanceContent(fileContent)
	enhancedLength := len(strings.Split(enhanced, "\n"))
	...
	fmt.Printf("✅ Enhanced: %s (%d → %d lines)\n", baseName, originalLength, enhancedLength)
}

func enhanceContent(content string) string {
	result := content

	// 1. Enhance English Cons section (add 3 more items before closing tag)
	result = enhanceCons(result)

	// 2. Insert Best For and Not Recommended sections (English)
	result = insertBestForSections(result)

	// 3. Enhance Value Analysis (English)
	result = enhanceValueAnalysis(result)

	// 4. Enhance Performance Rating (English)
	result = enhancePerformanceRating(result)

	// 5. Insert FAQ section (English)
	result = insertFAQ(result)

	// 6. Enhance Bengali Cons section
	result = enhanceConsBN(result)

	// 7. Insert Best For, Not Recommended, Value Analysis, Rating, FAQ (Bengali)
	result = insertBengaliSections(result)

	return result
}

func enhanceCons(content string) string {
	// Add 3 more cons items before the closing </ul> of cons section
	// Look for the pattern where cons section ends
	pattern := `</ul>
</section>

<section class="value-analysis-section section card">`

	if strings.Contains(content, pattern) && !strings.Contains(content, "Brand Recognition") {
		replacement := `<li class="con-item"><strong>Brand Recognition:</strong> Less established reputation</li>
...
func insertBestForSections(content string) string {
	// Insert Best For and Not Recommended sections before Value Analysis
	insertPoint := `<section class="value-analysis-section section card">`

	if !strings.Contains(content, "best-for-section") {
		newSections := `<section class="best-for-section section">
...
<section class="value-analysis-section section card">`
		...
		content = strings.Replace(content, insertPoint, newSections, 1)
	}
...
func enhanceValueAnalysis(content string) string {
	// Add 5 more value items before closing </div> of value analysis
	pattern := `</div>
</section>

<section class="performance-rating-section section">`

	if strings.Contains(content, pattern) && !strings.Contains(content, "Fuel Economy:</strong>") {
		replacement := `<p class="value-item"><strong>Fuel Economy:</strong> <span>45-48 km/l (mixed riding)</span></p>
...
func enhancePerformanceRating(content string) string {
	// This is more complex - need to find and replace the entire rating section
	// Look for the pattern with 5 ratings and expand to 10

	// Find performance rating section start
	startMarker := `<section class="performance-rating-section section">`
	endMarker := `</ul>
</section>
...
<section class="final-verdict-section section card highlight">`
	...
	if startIdx == -1 || strings.Contains(content, "Engine Performance:</strong>") {
		return content // Already enhanced or not found
	}

	// Find the title line
	titleStart := startIdx + len(startMarker)
	titleEnd := strings.Index(content[titleStart:], "</h2>")
	if titleEnd == -1 {
		return content
	}

	titleLine := content[titleStart : titleStart+titleEnd+5] // Include </h2>
...
<section class="final-verdict-section section card highlight">`
	...
	content = content[:startIdx] + newRatingSection + content[endIdx+len(endMarker):]

	return content
}

func insertFAQ(content string) string {
	// Insert FAQ before Final Verdict (English)
...
func enhanceConsBN(content string) string {
	// Add 3 more Bengali cons before the Bengali final verdict
	// Find Bengali cons section end
	pattern := `</ul>
</section>
...
<h2 class="section-title verdict-title">চূড়ান্ত সিদ্ধান্ত:`
	...
	if strings.Contains(content, pattern) && !strings.Contains(content, "ব্র্যান্ড স্বীকৃতি:") {
		replacement := `<li class="con-item"><strong>ব্র্যান্ড স্বীকৃতি:</strong> কম প্রতিষ্ঠিত সুনাম</li>
...
func insertBengaliSections(content string) string {
	// Insert all Bengali sections before Bengali Final Verdict
	insertPoint := `<section class="final-verdict-section section card highlight">
<h2 class="section-title verdict-title">চূড়ান্ত সিদ্ধান্ত:`

	if !strings.Contains(content, "সেরা জন্য") {
		bengaliSections := `<section class="best-for-section section">
...
<h2 class="section-title verdict-title">চূড়ান্ত সিদ্ধান্ত:`
		...
		content = strings.Replace(content, insertPoint, bengaliSections, 1)
	}
